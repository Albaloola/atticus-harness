...
- Deterministic context packs with stable prefix sections, evidence/artifact bundles, token estimates, fingerprints, prompt discipline, and cache accounting fields.
- First-class model routing for OpenRouter-hosted models, explicit OpenRouter fallback pools, and exact Codex GPT-5.5 policy with requested/actual model accounting.
...
- Prompt-bearing surfaces instruct workers that output is candidate, not canonical; facts, law, procedure, inference, contradiction, risk, drafting notes, and uncertainty must remain distinct and citation-bound.
...
atticus model-policy validate --policy-file tests/fixtures/model_policies/layered_openrouter_pool.json
