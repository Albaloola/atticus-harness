...
def test_cli_provider_probe_no_longer_requires_live_opt_in(capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-test")
    monkeypatch.delenv("ATTICUS_ENABLE_LIVE_OPENROUTER", raising=False)
    ...
    assert "ATTICUS_ENABLE_LIVE_OPENROUTER" not in str(output.get("reason", ""))


def test_cli_live_resume_no_longer_requires_live_opt_in(tmp_path: Path, capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch):
    db_path = init_db(tmp_path)
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-test")
    monkeypatch.delenv("ATTICUS_ENABLE_LIVE_OPENROUTER", raising=False)
    with repo.db_connection(db_path) as conn:
    ...
    assert code in (0, 1, 2)
    assert isinstance(output, dict)
    assert not any("ATTICUS_ENABLE_LIVE_OPENROUTER" in reason for reason in _strings(output.get("reasons", [])))
