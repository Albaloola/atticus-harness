...
def test_openrouter_probe_no_longer_requires_live_opt_in():
    from atticus.providers.live_readiness import live_openrouter_enabled
    assert live_openrouter_enabled() is True
...
def test_openrouter_client_requires_provider_model_metadata_for_fallback_detection():
    ...
    client = OpenRouterClient(REDACTED_CONFIG_VALUE transport=lambda req, timeout: json.dumps(raw).encode("utf-8"))
...
def test_openrouter_client_rejects_empty_content_message():
    raw = {
        "provider": "openrouter",
        "model": "deepseek/deepseek-v4-pro",
    ...
    test_api_key = "sk-" + "test"
    client = OpenRouterClient(REDACTED_CONFIG_VALUE transport=lambda req, timeout, raw=raw: json.dumps(raw).encode("utf-8"))

    with pytest.raises(OpenRouterError, match="content was empty"):
        _ = client.chat_json(model="deepseek/deepseek-v4-pro", messages=[{"role": "user", "content": "{}"}], max_tokens=8, temperature=0.0)


def test_openrouter_client_rejects_non_mapping_content_payload():
    raw = {
        "provider": "openrouter",
        "model": "deepseek/deepseek-v4-pro",
    ...
    with pytest.raises(OpenRouterError, match="content must be a JSON object"):
        _ = client.chat_json(model="deepseek/deepseek-v4-pro", messages=[{"role": "user", "content": "{}"}], max_tokens=8, temperature=0.0)
