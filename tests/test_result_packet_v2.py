...
def test_result_packet_v2_normalizes_atticus_working_artifact_path():
    ...
    artifacts[0]["path"] = "LOCAL_PATH_REDACTED/atticus-harness/matters/napier/03-working/draft-notes/path-task.md"
...
def test_external_action_request_is_blocked_and_quarantined(tmp_path: Path):
    ...
    with repo.db_connection(db_path) as conn:
        ...
        packet["external_action_requests"] = [{"action_type": "email", "recipient": "EMAIL_REDACTED"}]
        candidate_id = record_worker_result(
            conn,
