...
- Human-attention does not mean “make User debug the harness”. Internal failures are routed back to the scheduler, orchestrator, provider control plane, or reducer.
- The harness emits an agent handoff packet so OpenClaw, Hermes, Codex, or another runner knows when to ask User a clear question and when to keep working without interrupting him.
- Any answer from User is carried back through `human-response submit`, with optional files registered as supplemental sources.
...
- `needs_human=true`: ask User the provided question, then submit the answer using `on_answer` / `human-response submit`.
...
If User provides files, repeat `--file PATH`. The harness registers them as operator-submitted supplemental sources and links the response to the blocker.
