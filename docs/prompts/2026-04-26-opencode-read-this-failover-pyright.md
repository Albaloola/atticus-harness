...
You are working on Omar's Atticus legal harness. This is safety-sensitive legal automation.
...
The current failover implementation raises `OpenRouter failover exhausted` after `max_failed_cycles`. That is not the behavior Omar asked for.

Implement Omar's requested behavior exactly:
