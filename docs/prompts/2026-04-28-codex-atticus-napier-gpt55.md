...
Continue the Atticus harness wiring for User's Napier sister-case so the harness can support a first-class per-run model selection path, specifically provider `openai-codex` with model `gpt-5.5`, fallback disabled, and no accidental OpenRouter/free-model/DeepSeek fallback.

The goal is not a one-off SQL hack. User should be able to choose any supported model for a run or matter through a normal harness surface.
...
```text
Matter workspace:
  LOCAL_PATH_REDACTED/atticus-harness/matters/napier-accommodation-arrears

Matter DB:
  LOCAL_PATH_REDACTED/atticus-harness/data/napier-accommodation-arrears.sqlite

Rich local inventory:
  LOCAL_PATH_REDACTED/atticus-harness/matters/napier-accommodation-arrears/02-registers/file_inventory.csv

Sparse source register:
  LOCAL_PATH_REDACTED/atticus-harness/matters/napier-accommodation-arrears/02-registers/source_register.csv

Existing extracted text directory:
  LOCAL_PATH_REDACTED/atticus-harness/matters/napier-accommodation-arrears/03-working/extracted-text/
```
...
2. Do not start a live GPT-5.5/Codex run unless all explicit live gates are implemented, tests pass, no-live readiness passes, and User or Jake explicitly approves the live spend.
...
6. Provide exact no-live readiness commands and, if not actually run, the exact bounded live command User or Jake can run later.
...
or integrate this into seeding if that is cleaner, but User's requirement is broader than seeding. He should be able to run the harness on any supported model he chooses.
...
8. Exact command User or Jake should run next, but only as a command to run later unless live execution has been explicitly approved.
