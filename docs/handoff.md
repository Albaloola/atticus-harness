...
- Do not run live provider calls through agents unless User explicitly approves the exact live run.
- Do not let agents touch `LOCAL_PATH_REDACTED/.openclaw/workspace/`.
- Agents may inspect or edit `LOCAL_PATH_REDACTED/atticus-harness` and `LOCAL_PATH_REDACTED/.openclaw/workspace-atticus-legal/` only as required by the task.
...
atticus import-candidates --workspace LOCAL_PATH_REDACTED/.openclaw/workspace-atticus-legal --db /tmp/atticus-smoke.sqlite3 --dry-run
...
atticus migrate-report --workspace LOCAL_PATH_REDACTED/.openclaw/workspace-atticus-legal --db /tmp/atticus-smoke.sqlite3 --dry-run
