...
The harness monitor is an interactive terminal UI (and non-interactive JSON mode) for the Atticus legal harness. It gives a human operator — whether Omer or an agent — a single cockpit view of a matter without requiring knowledge of internal CLI commands.
...
- `needs_human=true` → The agent must ask Omer the supplied question
