...
from atticus.workers.result_parser import ResultPacketError, parse_result, sanitize_prose
...
def _artifact_content(*, packet: Mapping[str, object], proposed: Mapping[str, object], candidate_id: str) -> str:
    return json.dumps(
        {
            "candidate_id": candidate_id,
            "summary": sanitize_prose(str(packet.get("summary") or "")),
            "findings": packet.get("findings") or [],
            "citations": packet.get("citations") or [],
            "proposed_artifact": proposed,
            "proposed_content": sanitize_prose(str(proposed.get("content") or "")),
        },
        sort_keys=True,
        indent=2,
    )
