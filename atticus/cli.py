...
def build_parser() -> argparse.ArgumentParser:
    ...
    config = sub.add_parser("config", help="view and edit harness configuration")
    config_sub = config.add_subparsers(dest="config_action", required=True)

    config_show = config_sub.add_parser("show", help="display current configuration as JSON")
    _ = config_show.add_argument("--json", dest="json_output", action="store_true")

    config_set = config_sub.add_parser("set", help="set a configuration value")
    config_set_sub = config_set.add_subparsers(dest="config_set_target", required=True)

    config_set_model = config_set_sub.add_parser("model", help="set OpenRouter model for a tier")
    _ = config_set_model.add_argument("--tier", required=True, choices=["flash_worker", "pro_orchestrator", "codex_exact"])
    _ = config_set_model.add_argument("--model", required=True)

    config_set_skill = config_set_sub.add_parser("skill", help="toggle a skill on/off")
    _ = config_set_skill.add_argument("--skill-id", required=True)
    _ = config_set_skill.add_argument("--active", required=True, choices=["true", "false"])

    config_set_budget = config_set_sub.add_parser("budget", help="set a matter budget limit")
    _ = config_set_budget.add_argument("--matter", required=True)
    _ = config_set_budget.add_argument("--limit", required=True, type=float)

    config_export = config_sub.add_parser("export", help="export full configuration as JSON")
    _ = config_export.add_argument("--out", help="output file path (default: stdout)")

    config_import = config_sub.add_parser("import", help="import configuration from JSON file")
    _ = config_import.add_argument("file", help="JSON config file to import")
    ...
    add_evidence_ingest_subparser(sub)

    models = sub.add_parser("models", help="list or refresh the OpenRouter model registry")
    _ = models.add_argument("action", choices=["list", "refresh"])
    _ = models.add_argument("--provider", default="", help="filter by provider prefix (e.g. openai/, anthropic/)")
    _ = models.add_argument("--free-only", action="store_true", help="show free models only")
    _ = models.add_argument("--tools-only", action="store_true", help="show tool-calling models only")
    _ = models.add_argument("--vision-only", action="store_true", help="show vision-capable models only")
    _ = models.add_argument("--min-context", type=int, default=0, help="minimum context length filter")
    _ = models.add_argument("--json", dest="json_output", action="store_true", help="output as JSON")
    _ = models.add_argument("--search", default="", help="substring filter on model id or name")
...
def _main(args: CliArgs) -> int:
    ...
    if args.command == "tools":
        tools_payload = {"tools": [tool.metadata.as_dict() for tool in list_tools() if hasattr(tool, 'metadata') and not getattr(tool, 'hidden', False)]}
...
def _handle_models(args: CliArgs) -> int:
    from atticus.providers.openrouter_registry import get_registry

    registry = get_registry()
    if args.action == "refresh":
        count = registry.refresh()
        print_json({"action": "refresh", "model_count": count, "cache_age_seconds": registry.cache_age_seconds})
        return 0
    ...
    if args.search:
        ...
        entries = [e for e in entries if search_lower in e.id.lower() or search_lower in e.name.lower()]

    if args.json_output:
        result = {
            "model_count": len(entries),
            "total_models": registry.model_count,
            "cache_age_seconds": registry.cache_age_seconds,
            "filters": {
                "free_only": args.free_only,
                "tools_only": args.tools_only,
                "vision_only": args.vision_only,
        ...
        print(f"\nOpenRouter Models ({len(entries)} shown, {registry.model_count} total, cache age: {registry.cache_age_seconds:.0f}s)")
        if args.free_only or args.tools_only or args.vision_only or args.min_context or args.provider or args.search:
            active_filters = []
            if args.free_only:
                active_filters.append("free only")
            if args.tools_only:
                active_filters.append("tools required")
            if args.vision_only:
                active_filters.append("vision required")
            if args.min_context:
                active_filters.append(f"context >= {args.min_context}")
            if args.provider:
                active_filters.append(f"provider: {args.provider}")
            if args.search:
                active_filters.append(f"search: {args.search}")
            print(f"Filters: {', '.join(active_filters)}")
        print()
        for entry in entries:
            cost_str = "FREE" if entry.is_free else f"${entry.prompt_price_per_million:.2f}/${entry.completion_price_per_million:.2f}/M"
            caps = []
            if entry.supports_tools:
                caps.append("tools")
            if entry.supports_vision:
                caps.append("vision")
            if entry.supports_structured_outputs:
                caps.append("struct")
            cap_str = f" [{', '.join(caps)}]" if caps else ""
            ctx_str = f"{entry.context_length // 1000}K" if entry.context_length else "?"
            print(f"  {entry.id:<65s} {cost_str:<22s} ctx={ctx_str}{cap_str}")
        print()
    return 0
...
def _classify_live_resume_next_action(
    ...
    *,
    ...
    run_command = (
        f"python -m atticus.cli run-free-loop --db {db}{matter_arg} "
    ...
    if reducer is not None:
        ...
        if reducer_stage not in {"S6", "S7", "S8", "S9"}:
            return {
                ...
                "command": f"python -m atticus.cli run-free-loop --db {db}{matter_arg} --output-dir {output_dir} --capacity {capacity} --max-ticks 1 --runtime openrouter --allow-live",
                "task_id": _row_str(reducer, "task_id"),
...
def _handle_config_command(args: CliArgs) -> int:
    """Dispatch 'config' subcommands."""
    action = getattr(args, "config_action", "show")

    if action == "show":
        from atticus.operator_control import get_configuration_summary
        result = get_configuration_summary()
        print_json(result)
        return 0

    if action == "set":
        target = getattr(args, "config_set_target", "")
        return _handle_config_set(args, target)

    if action == "export":
        from atticus.config_manager import get_config_manager
        mgr = get_config_manager()
        config = mgr.load()
        output = json.dumps(config.as_dict(), indent=2, sort_keys=True) + "\n"
        out_path = getattr(args, "out", None)
        if out_path:
            Path(out_path).write_text(output, encoding="utf-8")
            print_json({"exported": str(out_path), "size_bytes": len(output)})
        else:
            sys.stdout.write(output)
        return 0

    if action == "import":
        from atticus.config_manager import get_config_manager, ConfigError
        file_path = getattr(args, "file", None)
        if not file_path:
            raise ValueError("config import requires FILE")
        if isinstance(file_path, list):
            file_path = file_path[0]
        try:
            raw_text = Path(str(file_path)).read_text(encoding="utf-8")
            raw = json.loads(raw_text)
            if not isinstance(raw, Mapping):
                raise ConfigError("imported config must be a JSON object")
            mgr = get_config_manager()
            config = mgr.load()
            # merge loaded values into a fresh config instance
            from atticus.config_manager import AtticusConfig, _DEFAULT_MODELS, _DEFAULT_PROVIDER_SETTINGS
            merged = AtticusConfig(
                version=int(raw.get("version", config.version)),
                models={**dict(_DEFAULT_MODELS), **dict(raw.get("models", {}))},
                skills=dict(raw.get("skills", {})),
                providers={**dict(_DEFAULT_PROVIDER_SETTINGS), **dict(raw.get("providers", {}))},
                budget={str(k): float(str(v)) for k, v in raw.get("budget", {}).items()},
            )
            mgr.save(merged)
            print_json({"imported": str(file_path), "ok": True})
        except (json.JSONDecodeError, ConfigError, OSError) as exc:
            print_json({"imported": str(file_path), "ok": False, "error": str(exc)})
            return 2
        return 0
...
def _handle_config_set(args: CliArgs, target: str) -> int:
    """Dispatch 'config set <target>' subcommands."""
    from atticus.operator_control import set_model_for_tier, set_skill_active, set_budget_limit

    if target == "model":
        tier = str(getattr(args, "tier", ""))
        model = str(getattr(args, "model", ""))
        result = set_model_for_tier(tier, model)
        print_json(result)
        return 0 if result.get("ok") else 1

    if target == "skill":
        skill_id = str(getattr(args, "skill_id", ""))
        active_str = str(getattr(args, "active", "true")).lower()
        active = active_str == "true"
        result = set_skill_active(skill_id, active)
        print_json(result)
        return 0 if result.get("ok") else 1

    if target == "budget":
        matter = str(getattr(args, "matter", ""))
        limit = float(str(getattr(args, "limit", "0.0")))
        result = set_budget_limit(matter, limit)
        print_json(result)
        return 0 if result.get("ok") else 1

    raise ValueError(f"unknown config set target: {target}")
