...
def parse_result(
    ...
    *,
    ...
    try:
        ...
        for finding in findings:
            if isinstance(finding.get("text"), str):
                finding["text"] = sanitize_prose(str(finding["text"]))
        for artifact in proposed_artifacts:
            if isinstance(artifact.get("content"), str):
                artifact["content"] = sanitize_prose(str(artifact["content"]))
    ...
    except (ResultPacketError, RecursionError) as exc:
        if isinstance(exc, RecursionError):
            raise ResultPacketError("result packet too deeply nested") from exc
        raise
...
def _normalize_proposed_artifact_path(path: str) -> str:
    ...
    examples, for example ``LOCAL_PATH_REDACTED/matters/<matter>/03-working/foo.md``.
