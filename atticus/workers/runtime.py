...
def execute_openrouter_work_order(
    ...
    *,
    ...
    try:
        ...
        # Re-verify lease still active (may have expired during execution)
        try:
            require_active_lease(conn, lease_id=lease_id, task_id=task_id)
        except Exception as exc:
            raise WorkerExecutionBlocked(f"lease expired during execution: {exc}") from exc
    ...
    *,
    ...
    if (
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            future = pool.submit(adapter.run, work_order, model=model, max_tokens=max_tokens, temperature=temperature)
            try:
                return future.result(timeout=timeout_seconds)
            except FuturesTimeoutError:
                future.cancel()
                raise ProviderCallTimeout(
                    f"OpenRouter provider call timed out after {timeout_seconds:g}s"
                ) from None
