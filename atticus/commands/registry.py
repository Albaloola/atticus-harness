...
def _commands() -> tuple[CommandDef, ...]:
    return (
        ...
        CommandDef("control-panel", "Human-facing matter control panel and agent handoff packet.", aliases=("panel",), read_only_safe=True),
        ...
        CommandDef("human-request", "Show structured human-facing requests from the harness.", read_only_safe=True),
        CommandDef("human-response", "Submit operator response to a harness request.", requires_write=True),
        ...
        CommandDef("repair-tick", "Execute one bounded deterministic repair continuation tick.", supports_dry_run=True),
        ...
        CommandDef("run", "Manage harness runs: stop, stop-current.", requires_write=True),
        ...
        CommandDef("source-led-packet", "Generate a deterministic quote-supported candidate from current source chunks.", supports_dry_run=True),
