...
def record_loop_guard_failure(
    ...
    *,
    ...
    # Safety net: count total errors for this target across ALL signatures.
    # consecutive_count resets when the error message changes, which allows
    # bypassing the 5/10/15 escalation limits by varying error messages.
    # This check uses the absolute total to force terminal escalation once the
    # overall error count reaches the limit, regardless of signature diversity.
    total_errors_row = conn.execute(
        "SELECT COUNT(*) AS n FROM error_logs WHERE target_type = ? AND target_id = ? AND error_type = ?",
        (target_type, target_id, clean_error_type),
    ).fetchone()
    total_error_count = (int(total_errors_row["n"]) if total_errors_row else 0) + 1
    if total_error_count >= ORCHESTRATOR_REPAIR_ATTEMPT_LIMIT and not bool(escalation.get("terminal")):
        escalation = {
            "signal_count": total_error_count,
            "escalation_level": LOOP_GUARD_MAX_AUTO_ESCALATION_LEVEL + 1,
