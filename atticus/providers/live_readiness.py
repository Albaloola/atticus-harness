...
def probe_live_openrouter(
    ...
    *,
    ...
    if client is not None:
        setattr(client, "timeout", 15.0)
    probe_client = cast(
        ChatJsonClient,
        openrouter_client_for_policy(provider_policy, env=env, live=True, client=client) or OpenRouterClient(REDACTED_CONFIG_VALUE ""), timeout=15.0),
    )
    try:
