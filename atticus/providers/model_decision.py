...
def _decision(
    ...
    if profile is None:
        return ModelDecision("blocked", "blocked", "blocked", "blocked", "no profile is available", BLOCKED_TIER, False, True, policy_fingerprint, input_fingerprint)
    return ModelDecision(
...
def _profile_for_tier(policy: ModelRoutingPolicyLike, tier: str) -> ModelProfileLike | None:
    ...
    if tier == FLASH_TIER:
        profile = _first_active(candidates, provider="openrouter", model="deepseek/deepseek-v4-flash")
        if profile is not None:
            return profile
        return _registry_best_for_tier(candidates, tier)
    if tier == PRO_TIER:
        profile = _first_active(candidates, provider="openrouter", model="deepseek/deepseek-v4-pro")
        if profile is not None:
            return profile
        return _registry_best_for_tier(candidates, tier)
    if tier == CODEX_TIER:
...
def _registry_best_for_tier(profiles: list[ModelProfileLike], tier: str) -> ModelProfileLike | None:
    registry = get_registry()
    if tier == FLASH_TIER:
        best = registry.cheapest_capable(min_context=32_000, supports_tools=True)
    elif tier == PRO_TIER:
        best = registry.best_reasoning(min_context=128_000)
    else:
        return None
    ...
    return _first_active(list(profiles), provider="openrouter", model=best.id)
