...
from atticus.providers.openrouter import (
    OpenRouterClient,
    OpenRouterError,
    validate_cache_usage_tokens,
...
ENV_FAILOVER_PER_MODEL_TIMEOUT_SECONDS = (
    "ATTICUS_OPENROUTER_FAILOVER_PER_MODEL_TIMEOUT_SECONDS"
)
...
DEFAULT_MAX_429_BACKOFF_SECONDS = 60.0
MAX_429_EXPONENT = 8  # 2^8 = 256x multiplier cap
...
class OpenRouterModelFailover:
        ...
        *,
        ...
        if max_total_attempts is not None and max_total_attempts < 1:
            raise OpenRouterFailoverHardError(
                "OpenRouter failover max_total_attempts must be >= 1 when set"
            )
        if max_total_attempts is None:
            max_total_attempts = len(self.config.models) * (
                self.config.max_failed_cycles + 1
            )
        failed_cycles = 0
        attempts_in_cycle = 0
        attempt_number = 0
        ...
        while True:
            if attempt_number >= max_total_attempts:
                ...
                raise OpenRouterError(
                    f"OpenRouter failover max_total_attempts exceeded after {attempt_number} attempts across {len(self.config.models)} configured models"
                )
            ...
            try:
                ...
                normalized = self._normalize_success_response(
                    response, requested_model=attempted_model
                )
    ...
    def _validate_request(self, *, messages: object) -> None:
        if not isinstance(messages, list) or not messages:
            raise OpenRouterFailoverHardError(
                "OpenRouter failover request must include non-empty messages"
            )
        message_items = cast(list[object], messages)
        for index, message in enumerate(message_items):
            if not isinstance(message, Mapping):
                raise OpenRouterFailoverHardError(
                    f"OpenRouter failover messages[{index}] must be a JSON object"
                )
            message_map = cast(Mapping[object, object], message)
            if not str(message_map.get("content") or "").strip():
                raise OpenRouterFailoverHardError(
                    f"OpenRouter failover messages[{index}] must include non-empty content"
                )
    ...
    def _client_for_attempt(self) -> ChatJsonClient:
        if self.client_factory is not None:
            return cast(
                ChatJsonClient,
                self.client_factory(self.config.per_model_timeout_seconds),
            )
    ...
    def _normalize_success_response(
        self, response: object, *, requested_model: str
    ) -> dict[str, object]:
    ...
    def _advance_after_failure(
        self, *, attempt_number: int, failed_cycle_count: int, reason: str
    ) -> None:
    ...
    def _sleep_between_failures(self) -> None:
        ...
        if self._consecutive_429s > 1:
            exponent = min(self._consecutive_429s, MAX_429_EXPONENT)
            delay = self.config.backoff_seconds * (2 ** (exponent - 1))
        if self.config.jitter_seconds > 0:
            delay += max(0.0, self.jitter(0.0, self.config.jitter_seconds))
        if self._consecutive_429s > 0:
            delay = min(delay, DEFAULT_MAX_429_BACKOFF_SECONDS)
        if delay > 0:
            self.sleep(delay)
...
def is_429_error(exc: OpenRouterError) -> bool:
    status_code = getattr(exc, "status_code", None)
    if status_code == 429:
        return True
    body = str(getattr(exc, "body", "") or "")
    text = f"{exc} {body}".lower()
    rate_limit_terms = ("rate limit", "rate_limit", "too many requests")
...
def classify_openrouter_failure(exc: OpenRouterError) -> tuple[bool, str]:
    ...
    if status_code in {400, 401, 402, 403, 413, 422} or any(
...
def openrouter_failover_config_from_policy(
    ...
    *,
    ...
    raw_config = (
        _mapping_to_dict(cast(Mapping[object, object], raw))
    ...
    config = OpenRouterFailoverConfig(
        provider=provider,
        models=models,
        max_failed_cycles=_int_value(
            raw_config.get("max_failed_cycles"),
            env.get(ENV_FAILOVER_MAX_FAILED_CYCLES),
            default=DEFAULT_MAX_FAILED_CYCLES,
        ),
        cooldown_seconds=_float_value(
            raw_config.get("cooldown_seconds"),
            env.get(ENV_FAILOVER_COOLDOWN_SECONDS),
            default=DEFAULT_COOLDOWN_SECONDS,
        ),
        per_model_timeout_seconds=_float_value(
            raw_config.get("per_model_timeout_seconds"),
            env.get(ENV_FAILOVER_PER_MODEL_TIMEOUT_SECONDS),
            default=DEFAULT_PER_MODEL_TIMEOUT_SECONDS,
        ),
        backoff_seconds=_float_value(
            raw_config.get("backoff_seconds"),
            env.get(ENV_FAILOVER_BACKOFF_SECONDS),
            default=DEFAULT_BACKOFF_SECONDS,
        ),
        jitter_seconds=_float_value(
            raw_config.get("jitter_seconds"),
            env.get(ENV_FAILOVER_JITTER_SECONDS),
            default=DEFAULT_JITTER_SECONDS,
        ),
        log_events=_bool_value(
            raw_config.get("log_events"),
            default=_env_bool(env.get(ENV_FAILOVER_LOG_EVENTS), default=False),
        ),
    )
    ...
    *,
    ...
    if client is not None:
        return OpenRouterModelFailover(
            config=config, client=client, event_sink=event_sink
        )
    env = env if env is not None else os.environ
    if event_sink is not None:
        return OpenRouterModelFailover(
            ...
            client=OpenRouterClient(
                ...
                timeout=config.per_model_timeout_seconds,
            ),
...
def openrouter_models_for_policy(
    provider_policy: Mapping[str, object],
    *,
    env: Mapping[str, str] | None = None,
    live: bool = False,
...
def _validate_failover_config(
    ...
    *,
    ...
    if config.provider != "openrouter":
        raise OpenRouterFailoverHardError(
            f"OpenRouter failover requires provider 'openrouter', got {config.provider!r}"
        )
    if not config.models:
        raise OpenRouterFailoverHardError(
            "OpenRouter failover requires at least one model"
        )
    unknown_models = [
        model
        for model in config.models
        if not known_model("openrouter", model, env=env, live=live)
    ]
    if unknown_models:
        raise OpenRouterFailoverHardError(
            f"OpenRouter failover models are unknown or unsupported: {', '.join(unknown_models)}"
        )
    if config.max_failed_cycles < 1:
        raise OpenRouterFailoverHardError(
            "OpenRouter failover max_failed_cycles must be >= 1"
        )
    if config.cooldown_seconds < 0:
        raise OpenRouterFailoverHardError(
            "OpenRouter failover cooldown_seconds must be non-negative"
        )
    if config.per_model_timeout_seconds <= 0:
        raise OpenRouterFailoverHardError(
            "OpenRouter failover per_model_timeout_seconds must be positive"
        )
    if config.backoff_seconds < 0:
        raise OpenRouterFailoverHardError(
            "OpenRouter failover backoff_seconds must be non-negative"
        )
    if config.jitter_seconds < 0:
        raise OpenRouterFailoverHardError(
            "OpenRouter failover jitter_seconds must be non-negative"
        )
...
def _models_value(value: object, *, source: str) -> tuple[str, ...]:
    if value is None:
        raise OpenRouterFailoverHardError(
            f"OpenRouter failover {source} models must be a non-empty string or list"
        )
    if isinstance(value, str):
        parsed_models = tuple(item.strip() for item in value.split(",") if item.strip())
        if not parsed_models:
            raise OpenRouterFailoverHardError(
                f"OpenRouter failover {source} models must not be empty"
            )
    ...
    if isinstance(value, list | tuple):
        ...
        for index, item in enumerate(items):
            if not isinstance(item, str) or not item.strip():
                raise OpenRouterFailoverHardError(
                    f"OpenRouter failover {source} models[{index}] must be a non-empty string"
                )
...
def _int_value(primary: object, fallback: object, *, default: int) -> int:
    ...
    if isinstance(raw, int | str):
        return int(raw)
    raise OpenRouterFailoverHardError(
        f"OpenRouter failover integer value is invalid: {raw!r}"
    )
...
def _float_value(primary: object, fallback: object, *, default: float) -> float:
    ...
    if isinstance(raw, int | float | str):
        ...
        if not math.isfinite(value):
            raise OpenRouterFailoverHardError(
                "OpenRouter failover float value must be finite"
            )
        return value
    raise OpenRouterFailoverHardError(
        f"OpenRouter failover float value is invalid: {raw!r}"
    )
