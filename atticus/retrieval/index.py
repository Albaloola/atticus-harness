...
def rebuild_search_index(
    ...
    *,
    ...
    _ = conn.execute("DELETE FROM search_index_entries WHERE index_name = ? AND matter_scope = ?", (index_name, matter_scope))
    for row in rows:
    ...
    output_rows: list[dict[str, object]] = [
        {"record_type": row["record_type"], "record_id": row["record_id"], "content_hash": row["content_hash"]}
        for row in conn.execute("SELECT record_type, record_id, content_hash FROM search_index_entries WHERE index_name = ? AND matter_scope = ? ORDER BY record_type, record_id", (index_name, matter_scope))
    ]
...
def _indexed_text(row: dict[str, object]) -> str:
