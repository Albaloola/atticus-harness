...
from atticus.cost_tracker import get_tracker as get_cost_tracker
...
_free_loop_lock = threading.Lock()
...
def run_free_loop(
    ...
    *,
    ...
    if matter_scope:
        if not _free_loop_lock.acquire(blocking=False):
            return {"ok": False, "ticks": [], "tick_count": 0, "stopped_by": "concurrent_execution_prevented"}
        try:
            conn.commit()
            conn.execute("PRAGMA busy_timeout = 1000")
            conn.execute("BEGIN EXCLUSIVE")
        except sqlite3.OperationalError:
            _free_loop_lock.release()
            return {"ok": False, "ticks": [], "tick_count": 0, "stopped_by": "concurrent_execution_prevented"}
    try:
        if max_ticks <= 0:
        ...
        for _ in range(max(0, max_ticks)):
            ...
            if not _tick_made_progress(tick):
                ...
                if action_type == "transient_error":
                    continue
...
def _execute_leased_workers(
    ...
    *,
    ...
    if db_path is None or len(leased_workers) <= 1:
        ...
        executor = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="atticus-worker")
        try:
            future_to_worker = {
                executor.submit(
            ...
            try:
                for future in as_completed(future_to_worker, timeout=300):
                    try:
                ...
                logging.warning(
                    "free_loop worker batch timed out after 300s; %d of %d futures still pending",
                    sum(1 for f in future_to_worker if not f.done()),
                    len(future_to_worker),
                )
...
def _execute_one_leased_worker(
    ...
    *,
    ...
    try:
        ...
        _record_worker_cost(conn, task_id, runtime)
...
def _matter_complete(conn: sqlite3.Connection, matter_scope: str | None) -> bool:
    ...
    try:
        ...
        logging.warning("_matter_complete check failed: %s", exc)
        _ = repo.emit_event(
            conn,
...
def _safe_next_action(conn: sqlite3.Connection, matter_scope: str | None) -> dict[str, object]:
    ...
    try:
        ...
        logging.warning("_safe_next_action check failed: %s", exc)
        _ = repo.emit_event(
            conn,
        ...
        return {"type": "transient_error", "owner": "system", "reason": f"_safe_next_action raised: {exc}"}
...
def _record_worker_cost(conn: sqlite3.Connection, task_id: str, runtime: str) -> None:
    try:
        row = conn.execute(
            "SELECT provider_policy_json FROM tasks WHERE task_id=?",
            (task_id,),
        ).fetchone()
        if not row or not row["provider_policy_json"]:
            return
        policy = json.loads(row["provider_policy_json"])
        model = str(policy.get("model") or "unknown")
        tracker = get_cost_tracker()
        tracker.add_usage(model, input_tokens=0, output_tokens=0)
    except Exception:
        pass
